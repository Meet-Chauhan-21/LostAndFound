...
const AdminPostPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log("Submitting admin post:", formData);
      const response = await api.post(`/user-reports/user-entry/${formData.email}`, formData);
      console.log("Admin post response:", response.data);
      ...
      if (response.data.includes("Success")) {
        ...
        Swal.fire({
          title: 'Error!',
          text: response.data || "Failed to create post",
          icon: 'error',
          background: '#1e213a',
          customClass: {
            popup: 'rounded-xl shadow-2xl',
      ...
      Swal.fire({
        title: 'Error!',
        text: 'Failed to create post: ' + (error.response?.data || error.message || "Please try again."),
        icon: 'error',
        background: '#1e213a',
        customClass: {
          popup: 'rounded-xl shadow-2xl',
