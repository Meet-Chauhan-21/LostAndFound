...
const PostLostModal = ({ isOpen, onClose }) => {
  ...
  const validateForm = async () => {
    if (!formData.itemName.trim()) {
      await Swal.fire({
        ...
        text: 'Please enter the name of the lost item',
    ...
    if (!formData.category) {
      await Swal.fire({
        ...
        text: 'Please select a category for the lost item',
        confirmButtonColor: '#8b5cf6',
        background: '#1e213a',
        backdrop: 'rgba(0,0,0,0.4)',
        customClass: {
    ...
    if (!formData.locationLost.trim()) {
      await Swal.fire({
        ...
        text: 'Please enter where you lost the item',
        confirmButtonColor: '#8b5cf6',
        background: '#1e213a',
        backdrop: 'rgba(0,0,0,0.4)',
        customClass: {
    ...
    if (!formData.dateLost) {
      await Swal.fire({
        icon: 'error',
        title: 'Missing Date',
        text: 'Please enter when you lost the item',
        confirmButtonColor: '#8b5cf6',
        background: '#1e213a',
        backdrop: 'rgba(0,0,0,0.4)',
        customClass: {
    ...
    if (!formData.description.trim()) {
      await Swal.fire({
        ...
        text: 'Please describe the lost item',
        confirmButtonColor: '#8b5cf6',
        background: '#1e213a',
        backdrop: 'rgba(0,0,0,0.4)',
        customClass: {
  ...
  const handleSubmit = async (e) => {
    ...
    const reportData = {
      ...
      itemLocation: formData.locationLost,
      itemDate: formData.dateLost,
      ...
      await Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'Failed to post lost item. Please try again.',
        confirmButtonColor: '#8b5cf6',
        background: '#1e213a',
        backdrop: 'rgba(0,0,0,0.4)',
        customClass: {
  ...
  return (
    <AnimatePresence mode="wait">
      {isOpen && (
        ...
        >
          <motion.div 
            className="bg-[#0c0e1d] rounded-xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden border border-purple-500/30"
          ...
          >
            <form onSubmit={handleSubmit} className="h-full flex flex-col">
              {/* Modal Header */}
              <motion.div 
                className="flex-shrink-0 p-6 border-b border-purple-500/30 flex justify-between items-center bg-gradient-to-r from-purple-600 to-blue-600 z-10"
          ...
          {/* Modal Content - Split Layout */}
            ...
            {/* Left Section - Item Details */}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({...formData, itemName: e.target.value})}
                          className="w-full p-3 border border-slate-600/50 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all bg-slate-800/60 backdrop-blur-sm text-white placeholder-slate-400"
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setFormData({...formData, category: e.target.value})}
                          className="w-full p-3 border border-slate-600/50 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all bg-slate-800/60 backdrop-blur-sm text-white"
                  >
              ...
              <div className="space-y-4">
                <div>
                        <label className="block text-slate-300 mb-2 font-medium">Location Lost *</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) => setFormData({...formData, locationLost: e.target.value})}
                            className="w-full pl-10 p-3 border border-slate-600/50 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all bg-slate-800 text-white placeholder-slate-400"
                ...
                <div>
                        <label className="block text-slate-300 mb-2 font-medium">Date Lost *</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      max={new Date().toISOString().split('T')[0]}
                            className="w-full pl-10 p-3 border border-slate-600/50 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all bg-slate-800 text-white"
                    />
                  </div>
                </div>
              </div>
            ...
            {/* Right Section - Image Upload & Contact Info */}
              ...
              <div>
                ...
                <label htmlFor="file-upload" className="cursor-pointer">
                        <div className="border-2 border-dashed border-slate-600/50 rounded-lg p-4 text-center hover:border-purple-400 transition-all bg-slate-800/60 backdrop-blur-sm">
                    {selectedImage ? (
